...
@Controller
@ModuleMapping(moduleId="contents", useSDesign=true)
@RequestMapping({"/{admSiteId}/menuContents/{usrSiteId}/contents", "/{admSiteId}/contents"})
public class ContentsController extends CommonController{
	...
	public String list(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) {
		...
		// 2.2 목록수
    	totalCount = contentsService.getTotalCount(lang, param);
    	...
    	if(totalCount > 0) {
    		...
    		// 2.3 목록
    		list = contentsService.getList(lang, param);
    	}
	...
	public String input(@RequestParam(value="mode") String mode, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		String contCd = StringUtil.getString(request.getParameter(JSONObjectUtil.getString(settingInfo, "idx_name")));

		if(!isModify || StringUtil.isEmpty(contCd)) {
	...
	public String inputProc(@RequestParam(value="mode") String mode, @ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		boolean isModify = false;
		if(StringUtil.isEquals(mode, "m")) isModify = true;
		String contCd = StringUtil.getString(request.getParameter(JSONObjectUtil.getString(attrVO.getSettingInfo(), "idx_name")));
		...
		// DB
    	int result = contentsService.update(lang, contCd, request.getRemoteAddr(), parameterMap, settingInfo, items, itemOrder);
    	if(result > 0) {
    		// 저장 성공
        	// 기본경로
        	fn_setCommonPath(attrVO);
	...
	public String input(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		/*
		 * branch option : branch type 가져오기 위함
		 * 현재 미사용 (branchType 항목 사용하는 경우 주석 제거)
		 * 기본 Branch Type 선택 -> 기본 Web1으로 저장
		JSONObject moduleItem = attrVO.getModuleItem();
		JSONObject branchItemInfo = JSONObjectUtil.getJSONObject(moduleItem, "branch_item_info");
	...
	public String inputProc(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
    	...
    	int result = contentsService.insert(request.getRemoteAddr(), parameterMap, settingInfo, items, itemOrder);
    	if(result > 0) {
    		// 저장 성공
        	// 기본경로
        	fn_setCommonPath(attrVO);
	...
	public String delete(@RequestParam(value="select") String[] deleteIdxs, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// DB
		int result = contentsService.delete(deleteIdxs, request.getRemoteAddr());
	...
	public String restore(@RequestParam(value="select") String[] restoreIdxs, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// DB
		int result = contentsService.restore(restoreIdxs, request.getRemoteAddr());
	...
	public String cdelete(@RequestParam(value="select") String[] deleteIdxs, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		int result = contentsService.cdelete(attrVO.getUploadModulePath(), deleteIdxs, items, itemOrder);
	...
	public String deleteList(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 2.2 목록수
    	totalCount = contentsService.getDeleteCount(lang, param);
		paginationInfo.setTotalRecordCount(totalCount);
    	...
    	if(totalCount > 0) {
    		...
    		// 2.3 목록
    		list = contentsService.getDeleteList(lang, param);
    	}
	 ...
	 * @param request
	 */
	public void fn_setManageListPath(HttpServletRequest request) {
		String baseQueryString = StringUtil.getString(request.getAttribute("allQueryString"));

		String branchListName = "branchList.do";
		String branchListUrl = branchListName;
		if(!StringUtil.isEmpty(baseQueryString)) {
			branchListUrl += baseQueryString;
		}

		request.setAttribute("URL_BRANCHDEFAULT_LIST", branchListUrl);
	}
