...
@ModuleMapping(moduleId="contactPage", confModule="contact")
public class ContactPageController extends ModuleController{
	...
	public String getStatsViewPath(String siteId, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) {
		...
		JSONObject itemInfo = JSONObjectUtil.getJSONObject(moduleItem, "site_item_info");
		...
		if(StringUtil.isEmpty(paramSearchType)) {
			...
			if(defaultSearchType > 0) {
				//Calendar nowThisDate  = Calendar.getInstance();
		...
		totalSum = contactService.getPageTotalSum(param);
		list = contactService.getPageList(param);
	...
	public String pageList(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) {
		String viewPath = getStatsViewPath(attrVO, request, model);
		if(!StringUtil.isEmpty(viewPath)) return viewPath;
    	// 4. 기본경로
    	fn_setCommonPath(attrVO);
		return getViewPath("/pageList");
	}
	...
	public void fn_setCommonPath(ModuleAttrVO attrVO) {	
		...
		PathUtil.fn_setListPath(queryString, baseParams, searchParams, pageName, "pageList.do");


		HttpServletRequest request = ((ServletRequestAttributes)RequestContextHolder.currentRequestAttributes()).getRequest();
		String baseQueryString = StringUtil.getString(request.getAttribute("allQueryString"));

		String excelDownName = "pageExcel.do";
	...
	@RequestMapping("/admPageList.do")
	public String admPageList(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) {
	...
	public void fn_setAdmCommonPath(ModuleAttrVO attrVO) {	
		...
		String pageName = JSONObjectUtil.getString(settingInfo, "page_name");		// 목록 페이징  key
		String[] searchParams = PathUtil.getSearchParams(JSONObjectUtil.getJSONObject(itemInfo, "list_search"));					// 목록 검색 항목
		PathUtil.fn_setListPath(queryString, baseParams, searchParams, pageName, "admPageList.do");


		HttpServletRequest request = ((ServletRequestAttributes)RequestContextHolder.currentRequestAttributes()).getRequest();
		String baseQueryString = StringUtil.getString(request.getAttribute("allQueryString"));

		String excelDownName = "admPageExcel.do";
		String excelDownUrl = excelDownName;
		if(!StringUtil.isEmpty(baseQueryString)) {
			excelDownUrl += baseQueryString;
		}
