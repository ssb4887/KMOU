...
import rbs.modules.busi.mapper.BusiFileMapper;
import rbs.modules.busi.mapper.BusiMapper;
import rbs.modules.busi.mapper.BusiMultiMapper;
...
public class BusiServiceImpl extends EgovAbstractServiceImpl implements BusiService {
	...
	private BusiMapper busiDAO;
	
	@Resource(name="busiFileMapper")
	private BusiFileMapper busiFileDAO;
	
	@Resource(name="busiMultiMapper")
	private BusiMultiMapper busiMultiDAO;
	...
	public int getTotalCount(int fnIdx, Map<String, Object> param) {
    	...
    	return busiDAO.getTotalCount(param);
    }
	...
	public DataMap getView(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
		DataMap viewDAO = busiDAO.getView(param);
		return viewDAO;
	}
	...
	public DataMap getFileView(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
		DataMap viewDAO = busiDAO.getFileView(param);
		return viewDAO;
	}
	...
	public DataMap getMultiFileView(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
		DataMap viewDAO = busiFileDAO.getFileView(param);
		return viewDAO;
	}
	...
	public DataMap getModify(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
		DataMap viewDAO = busiDAO.getModify(param);
		return viewDAO;
	}
	...
	public int getAuthCount(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
    	return busiDAO.getAuthCount(param);
    }
	...
	public int insert(String uploadModulePath, int fnIdx, String regiIp, ParamForm parameterMap, JSONObject settingInfo, JSONObject items, JSONArray itemOrder) throws Exception  {
		...
		// 1. key 얻기
		param.put("fnIdx", fnIdx);
		int keyIdx = busiDAO.getNextId(param);
		...
		// 3. DB 저장
		// 3.1 기본 정보 테이블
		int result = busiDAO.insert(param);
		...
		if(result > 0) {
			...
			if(multiDataList != null) {
				...
				for(int i = 0 ; i < multiDataSize ; i ++) {
					...
					fileParam.put("BUSI_IDX", keyIdx);
					result = busiMultiDAO.insert(fileParam);
				}
			}
			...
			if(fileDataList != null) {
				...
				for(int i = 0 ; i < fileDataSize ; i ++) {
					...
					fileParam.put("fnIdx", fnIdx);
					result = busiFileDAO.insert(fileParam);
				}
			}
		}
	...
	public int update(String uploadModulePath, int fnIdx, int keyIdx, String regiIp, ParamForm parameterMap, JSONObject settingInfo, JSONObject items, JSONArray itemOrder) throws Exception {
    	...
    	// 3. DB 저장
    	// 3.1 기본 정보 테이블
		int result = busiDAO.update(param);
		...
		if(result > 0){
			...
			result1 = busiMultiDAO.cdelete(multiDelParam);
			...
			if(multiDataList != null) {
				...
				for(int i = 0 ; i < multiDataSize ; i ++) {
					...
					multiParam.put("BUSI_IDX", keyIdx);
					result = busiMultiDAO.insert(multiParam);
				}
			}
			...
			if(fileDataList != null) {
				...
				fileParam1.put("BUSI_IDX", keyIdx);
				fileParam1.put("fnIdx", fnIdx);
				int fleIdx = busiFileDAO.getNextId(fileParam1);
			...
			if(fileModifyDataList != null) {
				...
				for(int i = 0 ; i < fileDataSize ; i ++) {
					...
					fileParam.put("BUSI_IDX", keyIdx);
					fileParam.put("fnIdx", fnIdx);
					result = busiFileDAO.update(fileParam);
				}
			}
			...
			if(fileDeleteSearchList != null) {
				...
				for(int i = 0 ; i < fileDataSize ; i ++) {
					...
					fileParam.put("BUSI_IDX", keyIdx);
					// 6.1 삭제목록 select
					List<Object> deleteFileList2 = busiFileDAO.getList(fileParam);
					if(deleteFileList2 != null) deleteMultiFileList.addAll(deleteFileList2);
					// 6.2 DB 삭제
					result = busiFileDAO.cdelete(fileParam);
				}
	...
	public int getDeleteCount(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
    	return busiDAO.getDeleteCount(param);
    }
	...
	public List<Object> getDeleteList(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
		return busiDAO.getDeleteList(param);
	}
	...
	public int cdelete(String uploadModulePath, int fnIdx, String[] deleteIdxs, JSONObject settingInfo, JSONObject items, JSONArray itemOrder) throws Exception {
		...
		deleteMultiFileList = busiFileDAO.getList(param);
		...
		if(deleteFileColumnList != null) {
			...
			deleteFileList = busiDAO.getFileList(param);
		}
		
		// 3. delete
		int result = busiDAO.cdelete(param);
		if(result > 0) {
			// 4. 파일 삭제
			// 4.1 multi file 삭제
			String fileRealPath = RbsProperties.getProperty("Globals.upload.file.path") + File.separator + uploadModulePath;
		...
		if(!JSONObjectUtil.isEmpty(itemOrder) && !JSONObjectUtil.isEmpty(items)) {
			...
			for(int i = 0 ; i < searchOrderSize ; i ++) {
				...
				if(formatType == 0 && objectType == 9) {
					...
					param.put("searchList", searchList);
			    	param.put("fnIdx", fnIdx);
					resultHashMap.put(itemId, busiFileDAO.getList(param));
				}
			}
		}
		...
		if(!JSONObjectUtil.isEmpty(itemOrder) && !JSONObjectUtil.isEmpty(items)) {
			...
			for(int i = 0 ; i < searchOrderSize ; i ++) {
				...
				if(formatType == 0 && (objectType == 3 || objectType == 4 || objectType == 11)) {
					...
					param.put("searchList", searchList);
			    	param.put("fnIdx", fnIdx);
					resultHashMap.put(itemId, busiMultiDAO.getList(param));
				}
			}
		}
