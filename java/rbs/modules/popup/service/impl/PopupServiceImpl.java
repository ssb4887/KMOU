package rbs.modules.popup.service.impl;
...
import rbs.modules.popup.mapper.PopupMapper;
import rbs.modules.popup.service.PopupService;

/**
 * 팝업관리에 관한 구현클래스를 정의한다.
 * @author user
 *
 */
@Service("popupService")
public class PopupServiceImpl extends EgovAbstractServiceImpl implements
		PopupService {
	...
	public int getTotalCount(int fnIdx, Map<String, Object> param) {
    	...
    	return popupDAO.getTotalCount(param);
    }
	...
	public DataMap getFileView(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
    	DataMap viewDAO = popupDAO.getFileView(param);
		return viewDAO;
	}
	...
	public DataMap getView(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
    	DataMap viewDAO = popupDAO.getView(param);
		return viewDAO;
	}
	...
	public DataMap getModify(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
    	DataMap viewDAO = popupDAO.getModify(param);
		return viewDAO;
	}
	...
	public int insert(String uploadModulePath, int fnIdx, String regiIp, ParamForm parameterMap, JSONObject settingInfo, JSONObject items, JSONArray itemOrder) throws Exception  {
		...
		// 1. key 얻기
		int brdIdx = popupDAO.getNextId(param);
    	...
    	dataList.add(new DTForm("POP_IDX", brdIdx));
    	...
    	// 4. DB 저장
		int result = popupDAO.insert(param);
	...
	public int update(String uploadModulePath, int fnIdx, int brdIdx, String regiIp, ParamForm parameterMap, JSONObject settingInfo, JSONObject items, JSONArray itemOrder) throws Exception {
    	...
    	// 1. 검색조건 setting
		searchList.add(new DTForm("POP_IDX", brdIdx));
		...
		// 3. DB 저장
		int result = popupDAO.update(param);
	...
	public int getDeleteCount(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
    	return popupDAO.getDeleteCount(param);
    }
	...
	public List<Object> getDeleteList(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
		return popupDAO.getDeleteList(param);
	}
	...
	public int delete(int fnIdx, String[] deleteIdxs, String regiIp) throws Exception {
		...
		// 1. 저장조건
		searchList.add(new DTForm("POP_IDX", deleteIdxs));
	...
	public int restore(int fnIdx, String[] restoreIdxs, String regiIp) throws Exception {
		...
		// 1. 저장조건
		searchList.add(new DTForm("POP_IDX", restoreIdxs));
	...
	public int cdelete(String uploadModulePath, int fnIdx, String[] deleteIdxs, JSONObject items, JSONArray itemOrder) throws Exception {
		...
		if(deleteFileColumnList != null) {
			...
			deleteFileList = popupDAO.getDeleteFileList(param);
		}
		
		// 3. delete
		int result = popupDAO.cdelete(param);
		if(result > 0) {
			// 4. 파일(단일항목) 삭제
			if(deleteFileList != null) {
				String fileRealPath = RbsProperties.getProperty("Globals.upload.file.path") + File.separator + uploadModulePath;
