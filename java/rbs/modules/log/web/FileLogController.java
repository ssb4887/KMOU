...
@ModuleMapping(moduleId="fileLog", confModule="log")
public class FileLogController extends ModuleController{
	...
	public String getStatsViewPath(boolean isExceldown, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) {
		...
		// 2.2 목록수
		totalCount = fileLogService.getTotalCount(param);
		if(!isExceldown) {
	    	...
	    	if(totalCount > 0) {
	    		...
	    		// 2.3 목록
	    		list = fileLogService.getList(param);
	    	}
		} else {
			// 엑셀다운로드시 사용하는 목록
	...
	public String logList(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) {
		...
		return getViewPath("/fileLogList");
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/fileLogExcel.do")
	public String excelList(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		return getViewPath("/fileLogExcel");
	}
	...
	public void fn_setCommonPath(ModuleAttrVO attrVO) {	
		...
		PathUtil.fn_setListPath(queryString, baseParams, searchParams, pageName, "fileLogList.do");


		HttpServletRequest request = ((ServletRequestAttributes)RequestContextHolder.currentRequestAttributes()).getRequest();
		String baseQueryString = StringUtil.getString(request.getAttribute("allQueryString"));

		String excelDownName = "fileLogExcel.do";
