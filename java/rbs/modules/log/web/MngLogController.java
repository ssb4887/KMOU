...
@ModuleMapping(moduleId="mngLog", confModule="log")
public class MngLogController extends ModuleController{
	
	@Resource(name="mngLogService")
	MngLogService mngLogService;
	...
	public String getStatsViewPath(boolean isExceldown, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) {
		...
		// 2.2 목록수
		totalCount = mngLogService.getTotalCount(param);
		if(!isExceldown) {
	    	...
	    	if(totalCount > 0) {
	    		...
	    		// 2.3 목록
	    		list = mngLogService.getList(param);
	    	}
		} else {
			// 엑셀다운로드시 사용하는 목록
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/mngLogExcel.do")
	public String excelList(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		return getViewPath("/mngLogExcel");
	}
	...
	public void fn_setCommonPath(ModuleAttrVO attrVO) {	
		...
		PathUtil.fn_setListPath(queryString, baseParams, searchParams, pageName, "mngLogList.do");


		HttpServletRequest request = ((ServletRequestAttributes)RequestContextHolder.currentRequestAttributes()).getRequest();
		String baseQueryString = StringUtil.getString(request.getAttribute("allQueryString"));

		String excelDownName = "mngLogExcel.do";
