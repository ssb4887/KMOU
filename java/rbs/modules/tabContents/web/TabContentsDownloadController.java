...
public class TabContentsDownloadController extends ModuleDownloadController{
	...
	public ModelAndView download(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		int calIdx = StringUtil.getInt(request.getParameter(JSONObjectUtil.getString(settingInfo, "idx_name")), 0);
		...
		if(StringUtil.isEmpty(itemId) || calIdx <= 0) {
		...
		searchList.add(new DTForm("A.CLASS_IDX", calIdx));
		if(fidx > 0) {
			// multi file
			columnId = "FILE";
			...
			dt = tabContentsService.getMultiFileView(fnIdx, param);
		} else {
			...
			param.put("searchList", searchList);
			dt = tabContentsService.getFileView(fnIdx, param);
		}
