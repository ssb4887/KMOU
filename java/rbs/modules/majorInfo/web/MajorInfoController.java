...
@RequestMapping("/{siteId}/majorInfo")
public class MajorInfoController extends ModuleController {
...
//		
//		//상세검색 코드 불러오기
//		String dept_campus = request.getParameter("dept_campus");
//		//대학코드
//		if("" != dept_campus && null != dept_campus) {
//			List<Object> univCode = codeOptnServiceOra.getHaksaCode("0000587");
...
//    	model.addAttribute("mjCdList", CodeHelper.getItemOptnHashMap(items, itemOrder));		// 전공코드
	...
	public String majorList(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		String yy = StringUtil.getString(request.getParameter("year"));
...
//		int keyIdx = StringUtil.getInt(request.getParameter(JSONObjectUtil.getString(settingInfo, "idx_name")), 0);
	 ...
	 */
	@ModuleAuth(name="LST")
	@RequestMapping(value = "/trackList.do")
	public String trackList(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		DataForm queryString = attrVO.getQueryString();
		boolean isAdmMode = attrVO.isAdmMode(); // 관리자면 true, 사용자면 false
		int fnIdx = attrVO.getFnIdx();
		JSONObject settingInfo = attrVO.getSettingInfo();
...
//		searchList.add(new DTForm("C." + idxStatisticColumnName, keyStatisticIdx));
		...
		// 2.2 목록수
//		totalCount = majorInfoService.getTotalCount(fnIdx, param);
		trackCount = majorInfoService.getTrackCount(fnIdx, param);
		
		paginationInfo.setTotalRecordCount(trackCount);
		...
		if(trackCount > 0) {
			...
			// 2.3 목록
			list = majorInfoService.getTrackList(fnIdx, param);
		}
...
//		totalCount = majorInfoService.getTotalCount(fnIdx, param);
	...
	public String view(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		String keyMajorIdx = StringUtil.getString(request.getParameter(JSONObjectUtil.getString(settingInfo, "idx_major_name")));
		int keyYearIdx = StringUtil.getInt(request.getParameter(JSONObjectUtil.getString(settingInfo, "idx_year_name")), 0);
		if(keyMajorIdx == null || keyYearIdx <= 0 ) {
		...
		// 상세정보
		dt = majorInfoService.getView(fnIdx, param);
		if(dt == null) {
			// 해당글이 없는 경우
...
//    	fn_setCommonPath(attrVO);
	...
	public String abilityInput(@RequestParam(value="mode") String mode, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		String keyMajorIdx = StringUtil.getString(request.getParameter(JSONObjectUtil.getString(settingInfo, "idx_major_name")));
		int keyYearIdx = StringUtil.getInt(request.getParameter(JSONObjectUtil.getString(settingInfo, "idx_year_name")), 0);
		int keyJobIdx = StringUtil.getInt(request.getParameter(JSONObjectUtil.getString(settingInfo, "idx_job_name")), 0);
...
//    	result = majorInfoService.updateYear(uploadModulePath, fnIdx, keyMajorIdx, keyYearIdx, request.getRemoteAddr(), parameterMap, settingInfo, items, itemYearOrder);
	...
	@ModuleAuth(name={"WRT"})
	@RequestMapping(method=RequestMethod.POST, value = "/trackInputProc.do", params="mode")
	public String trackInputProc(@RequestParam(value="mode") String mode, @ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		boolean isAjax = attrVO.isAjax();
		JSONObject settingInfo = attrVO.getSettingInfo();
		JSONObject itemInfo = attrVO.getItemInfo();
		String ajaxPName = attrVO.getAjaxPName();
...
//		int keyIdx = StringUtil.getInt(request.getParameter(JSONObjectUtil.getString(settingInfo, "idx_name")), 0);
		...
		String inputFlag = "modify";		// 항목설정 중 write_type, modify_type 구분
		JSONObject items = JSONObjectUtil.getJSONObject(itemInfo, "items");
		JSONArray itemTrackOrder = JSONObjectUtil.getJSONArray(itemInfo, submitType + "Trackproc_order");
		parameterMap.put("_itemInputTypeFlag", submitType);
		
		
		// 3.2 필수입력 체크
		String errorMessage = FormValidatorUtil.getErrorMessage(ajaxPName, parameterMap, moduleValidator, new Object[]{isAdmMode, inputFlag, items, itemTrackOrder, settingInfo});
		if(!StringUtil.isEmpty(errorMessage)) {
			model.addAttribute("message", errorMessage);
			return RbsProperties.getProperty("Globals.fail" + ajaxPName + ".path");
		}
...
//		result = majorInfoService.updateYear(uploadModulePath, fnIdx, keyMajorIdx, keyYearIdx, request.getRemoteAddr(), parameterMap, settingInfo, items, itemYearOrder);
		int result = majorInfoService.updateTrack(uploadModulePath, fnIdx, keyMajorIdx, keyYearIdx, keyTrackIdx, request.getRemoteAddr(), parameterMap, settingInfo, items, itemTrackOrder);
...
//		int result = 1;
		if(result > 0) {
			...
			String inputNpname = fn_dsetInputNpname(settingInfo);
			model.addAttribute("message", MessageUtil.getAlertAddWindow(isAjax, rbsMessageSource.getMessage("message.insert"), "fn_procAction(\"" + PathUtil.getAddProtocolToPagePath(StringUtil.getString(request.getAttribute("URL_TRACK_" + inputNpname))) + "\", " + StringUtil.getInt(request.getParameter("mdl")) + ");"));
			return RbsProperties.getProperty("Globals.message" + ajaxPName + ".path");
		} else if(result == -1) {
			// 4.2 파일업로드 오류
			String fileFailView = getFileFailViewPath(request);
...
//		int result = 1;
	 ...
	 */
	@ModuleAuth(name="WRT")
	@RequestMapping(method=RequestMethod.GET, value = "/copyProc.do")
	public String copyProc(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		JSONObject items = JSONObjectUtil.getJSONObject(itemInfo, "items");
		JSONArray itemOrder = JSONObjectUtil.getJSONArray(itemInfo, submitType + "proc_order");
		JSONArray itemCYearOrder = JSONObjectUtil.getJSONArray(itemInfo, submitType + "CYearproc_order");
		JSONArray itemStatisticOrder = JSONObjectUtil.getJSONArray(itemInfo, submitType + "Statisticproc_order");
		JSONArray itemCJobOrder = JSONObjectUtil.getJSONArray(itemInfo, submitType + "CJobproc_order");
		JSONArray itemCDtlOrder = JSONObjectUtil.getJSONArray(itemInfo, submitType + "CDtlproc_order");
		JSONArray itemCTrackOrder = JSONObjectUtil.getJSONArray(itemInfo, submitType + "CTrackproc_order");
		...
		// 여기서 다음년도 가져와서 insert 하기 전 !!!!!!!!!!!!!!!!!
		Map<String, Object> findParam = new HashMap<String, Object>();					// mapper parameter 데이터
		findParam.put("majorIdx", parameterMap.get("majorIdx"));
		
		int etcCd = majorInfoService.getMajorEtcCount(fnIdx, findParam);
		System.out.println("etcCd : " + etcCd);
		
		int copyTrack = 0;
		int copyYear = majorInfoService.copyYear(uploadModulePath, fnIdx, request.getRemoteAddr(), parameterMap, settingInfo, items, itemCYearOrder, Integer.parseInt((String) parameterMap.get("majorIdx")), Integer.parseInt((String) parameterMap.get("yearIdx")) );
		if(etcCd <1) {
			int copyStatistic = majorInfoService.copyStatistic(uploadModulePath, fnIdx, request.getRemoteAddr(), parameterMap, settingInfo, items, itemStatisticOrder, Integer.parseInt((String) parameterMap.get("majorIdx")), Integer.parseInt((String) parameterMap.get("yearIdx")) , copyYear );
			int copyJob = majorInfoService.copyJob(uploadModulePath, fnIdx, request.getRemoteAddr(), parameterMap, settingInfo, items, itemCJobOrder, Integer.parseInt((String) parameterMap.get("majorIdx")), Integer.parseInt((String) parameterMap.get("yearIdx")) , copyYear );
			copyTrack = majorInfoService.copyTrack(uploadModulePath, fnIdx, request.getRemoteAddr(), parameterMap, settingInfo, items, itemCTrackOrder, Integer.parseInt((String) parameterMap.get("majorIdx")), Integer.parseInt((String) parameterMap.get("yearIdx")) , copyYear );
			int copyDtl = majorInfoService.copyDtl(uploadModulePath, fnIdx, request.getRemoteAddr(), parameterMap, settingInfo, items, itemCDtlOrder, Integer.parseInt((String) parameterMap.get("majorIdx")), Integer.parseInt((String) parameterMap.get("yearIdx")) , copyYear );
			
		}
		
		System.out.println("copyYear :" + copyYear); // 복사할 때 넣은 YEAR_IDX 값
		
    	if(copyTrack > 0 || (etcCd > 0 && copyYear > 0 )) {
    		// 2.1 저장 성공
        	// 기본경로
        	fn_setCommonPath(attrVO);
...
//        	String inputNpname = fn_dsetInputNpname(settingInfo);
			...
			toUrl += "&" + JSONObjectUtil.getString(settingInfo, "idx_major_name") + "=" + parameterMap.get("mjCd")
	...
	@ModuleAuth(name="WRT")
	@RequestMapping(method=RequestMethod.POST, value = "/trackInputProc.do")
	public String trackInputProc(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		boolean isAjax = attrVO.isAjax();
		boolean isAdmMode = attrVO.isAdmMode();
		JSONObject settingInfo = attrVO.getSettingInfo();
		JSONObject itemInfo = attrVO.getItemInfo();
		...
		System.out.println("trackInputProc.do");
		...
		String inputFlag = "write";			// 항목설정 중 write_type, modify_type 구분
		JSONObject items = JSONObjectUtil.getJSONObject(itemInfo, "items");
		JSONArray itemOrder = JSONObjectUtil.getJSONArray(itemInfo, submitType + "Trackproc_order");
		parameterMap.put("_itemInputTypeFlag", submitType);
		...
		int track_idx = majorInfoService.insertTrack(uploadModulePath, fnIdx, request.getRemoteAddr(), parameterMap, settingInfo, items, itemOrder, StringUtil.getInt(majorIdx), StringUtil.getInt(yearIdx));
		if(track_idx > 0) {
			// 2.1 저장 성공
			// 기본경로
			fn_setCommonPath(attrVO);
//        	String inputNpname = fn_dsetInputNpname(settingInfo);
	...
	@ModuleAuth(name="WRT")
	@RequestMapping(method=RequestMethod.POST, value = "/jobInputProc.do")
	public String jobInputProc(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		boolean isAjax = attrVO.isAjax();
		boolean isAdmMode = attrVO.isAdmMode();
		JSONObject settingInfo = attrVO.getSettingInfo();
		JSONObject itemInfo = attrVO.getItemInfo();
		...
		System.out.println("jobInputProc.do");
		...
		System.out.println("majorIdx : " + majorIdx);
		System.out.println("yearIdx : " + yearIdx);
		
		int job_idx = majorInfoService.insertJob(uploadModulePath, fnIdx, request.getRemoteAddr(), parameterMap, settingInfo, items, itemJobOrder, StringUtil.getInt(majorIdx), StringUtil.getInt(yearIdx));
		int dtl_idx = majorInfoService.insertDtl(uploadModulePath, fnIdx, request.getRemoteAddr(), parameterMap, settingInfo, items, itemDtlOrder, StringUtil.getInt(majorIdx), StringUtil.getInt(yearIdx), job_idx);
		if(dtl_idx > 0) {
			// 2.1 저장 성공
			// 기본경로
			fn_setCommonPath(attrVO);
//        	String inputNpname = fn_dsetInputNpname(settingInfo);
			...
			String inputNpname = fn_dsetInputNpname(settingInfo);
			model.addAttribute("message", MessageUtil.getAlertAddWindow(isAjax, rbsMessageSource.getMessage("message.insert"), "fn_procAction(\"" + PathUtil.getAddProtocolToPagePath(StringUtil.getString(request.getAttribute("URL_JOB_" + inputNpname))) + "\", " + StringUtil.getInt(request.getParameter("mdl")) + ");"));
	...
	public String delete(@ParamMap ParamForm parameterMap, @RequestParam(value="select") int[] deleteIdxs, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 1. DB
		int result = majorInfoService.delete(fnIdx, parameterMap, deleteIdxs, request.getRemoteAddr(), settingInfo);
	...
	@ModuleAuth(name="WRT")
	@RequestMapping(method=RequestMethod.GET, value = "/deleteTrackProc.do")
	public String deleteTrack(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		boolean isAjax = attrVO.isAjax();
		String ajaxPName = attrVO.getAjaxPName();
		JSONObject settingInfo = attrVO.getSettingInfo();
		int fnIdx = attrVO.getFnIdx();
		...
		int result = majorInfoService.deleteTrack(fnIdx, parameterMap, keyMajorIdx, keyYearIdx, keyTrackIdx, request.getRemoteAddr(), settingInfo);
...
//			fn_setCommonPath(attrVO);
			...
			model.addAttribute("message",   MessageUtil.getAlertAddWindow(isAjax, rbsMessageSource.getMessage("message.delete"), "fn_procAction('" + PathUtil.getAddProtocolToPagePath(StringUtil.getString(request.getAttribute("URL_TRACK_LIST"))) + "', " + StringUtil.getInt(request.getParameter("mdl")) + ");"));
			return RbsProperties.getProperty("Globals.message" + ajaxPName + ".path");
		}
		// 3.2 저장 실패
		model.addAttribute("message", MessageUtil.getAlert(isAjax, rbsMessageSource.getMessage("message.no.delete")));
	...
	public String restore(@RequestParam(value="select") int[] restoreIdxs, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 1. DB
		int result = majorInfoService.restore(fnIdx, restoreIdxs, request.getRemoteAddr(), settingInfo);
	...
	public String cdelete(@RequestParam(value="select") int[] deleteIdxs, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		JSONArray itemOrder = JSONObjectUtil.getJSONArray(itemInfo, submitType + "_order");
		// 1.2 삭제처리
		int result = majorInfoService.cdelete(uploadModulePath, fnIdx, deleteIdxs, settingInfo, items, itemOrder);
	...
	public String deleteList(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 2.2 목록수
    	totalCount = majorInfoService.getDeleteCount(fnIdx, param);
		paginationInfo.setTotalRecordCount(totalCount);
    	...
    	if(totalCount > 0) {
    		...
    		// 2.3 목록
    		list = majorInfoService.getDeleteList(fnIdx, param);
    	}
...
//		trackListName += (baseQueryString + "&majorIdx=" + majorIdx + "&yearIdx=" + yearIdx);
		majorListName += (baseQueryString + "&mjCd=" + majorIdx + "&year=" + yearIdx);
		majorInputName += (baseQueryString + "&mjCd=" + majorIdx + "&year=" + yearIdx);
		jobListName += (baseQueryString + "&majorIdx=" + majorIdx + "&yearIdx=" + yearIdx);
		inputName += baseQueryString;
		modifyName += (baseQueryString + "&mode=m&mjCd=" + majorIdx + "&year=" + yearIdx);
		inputProcName += baseQueryString;
		...
		abilityListName += (baseQueryString + "&mjCd=" + majorIdx + "&year=" + yearIdx);
		abilityInputName += (baseQueryString + "&mode=m&mjCd=" + majorIdx + "&year=" + yearIdx);
		abilityInputProcName += (baseQueryString + "&mode=m&mjCd=" + majorIdx + "&year=" + yearIdx);
