...
<td<c:if test="${colspan > 0}"> colspan="${colspan}"</c:if>>
	<itui:objectPYMDHM itemId="${itemId}" itemInfo="${itemInfo}" objDt="${objDt}" objVal11="${objVal11}" objVal12="${objVal12}" objVal13="${objVal13}" objVal21="${objVal21}" objVal22="${objVal22}" objVal23="${objVal23}" btnId="${btnId}" btnClass="${btnClass}" btnText="${btnText}" objClass="${objClass}" objStyle="${objStyle}"/>
</td>
</c:if>