...
<c:if test="${!empty itemObj}">
	...
	<%
		...
		request.setAttribute("nowDate", calendar.get(Calendar.DATE));
	%>
	...
	<c:if test="${itemObj.default_current == 1}">
		...
		<c:set var="defVal3" value="${nowDate}"/>
	</c:if>
...
<c:forEach var="optnMonth" begin="1" end="12">
	...
	<option value="<c:out value="${optnMonthStr}"/>"<c:if test="${optnMonthStr == objVal2 || empty objVal2 && optnMonthStr == defVal2}"> selected="selected"</c:if>><c:out value="${optnMonthStr}"/></option>
</c:forEach>
</select>${itemMonthName}
<select id="${itemId}3" name="${itemId}3" style="${objStyle}" title="${dateObjTitle}"<c:if test="${required == 1}"> required="required"</c:if>>
...
<c:forEach var="optnDate" begin="1" end="31">
	...
	<option value="<c:out value="${optnDateStr}"/>"<c:if test="${optnDateStr == objVal3 || empty objVal3 && optnDateStr == defVal3}"> selected="selected"</c:if>><c:out value="${optnDateStr}"/></option>
</c:forEach>
</select>${itemDateName}
</c:if>