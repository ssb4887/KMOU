...
<c:if test="${empty isSearchList}">
	...
	<c:if test="${!empty itemListSearch}">
		...
		<c:if test="${!empty itemSearchOrder}">
			...
			<div class="${divClass}">
			<form name="${formName}" id="${formId}" method="get"  action="<c:out value="${formAction}"/>" style="width:100%; height:47px; display:flex; gap:.25rem !important">
				<elui:hiddenInput inputInfo="${queryString}" exceptNames="${searchFormExceptParams}"/>
				<itui:searchFormItemInBoard itemListSearch="${itemListSearch}" searchOptnHashMap="${searchOptnHashMap}" isUseRadio="${isUseRadio}" isUseMoreItem="${isUseMoreItem}"/>
				<jsp:doBody/>
