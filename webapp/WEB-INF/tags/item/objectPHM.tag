...
<c:if test="${!empty itemObj}">
	...
	<select id="${itemId}11" name="${itemId}11" title="${hourObjBeginTitle}"<c:if test="${required == 1}"> required="required"</c:if>>
		...
		<option value="${dateHourStr}"<c:if test="${objVal11 == dateHourStr}"> selected="selected"</c:if>>${dateHourStr}</option>
		</c:forEach>
	</select>
	:
	<select id="${itemId}12" name="${itemId}12" title="${minuteObjBeginTitle}"<c:if test="${required == 1}"> required="required"</c:if>>
		...
		<option value="${dateMinStr}"<c:if test="${objVal12 == dateMinStr}"> selected="selected"</c:if>>${dateMinStr}</option>
		</c:forEach>
	</select>
	~
	<select id="${itemId}21" name="${itemId}21" title="${hourObjEndTitle}"<c:if test="${required == 1}"> required="required"</c:if>>
		<option value="">${hourOptTitle}</option>
		<c:forEach var="dateHour" begin="0" end="23" varStatus="mi">
		<c:set var="dateHourStr" value="${elfn:getLPAD(dateHour, '0', 2)}"/>
		<option value="${dateHourStr}"<c:if test="${objVal21 == dateHourStr}"> selected="selected"</c:if>>${dateHourStr}</option>
		</c:forEach>
	</select>
	:
	<select id="${itemId}22" name="${itemId}22" title="${minuteObjEndTitle}"<c:if test="${required == 1}"> required="required"</c:if>>
		<option value="">${minuteOptTitle}</option>
		<c:forEach var="dateMin" begin="0" end="59" varStatus="mi">
		<c:set var="dateMinStr" value="${elfn:getLPAD(dateMin, '0', 2)}"/>
		<option value="${dateMinStr}"<c:if test="${objVal22 == dateMinStr}"> selected="selected"</c:if>>${dateMinStr}</option>
		</c:forEach>
	</select>
</c:if>