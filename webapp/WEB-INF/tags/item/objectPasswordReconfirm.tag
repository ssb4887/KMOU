...
<c:if test="${!empty itemObj}">
	...
	<input type="password" id="${itemId}_reconfirm" name="${itemId}_reconfirm" autocomplete="off" class="${objClass}" style="${objStyle}"<c:if test="${itemObj['maximum'] > 0}"> maxlength="${itemObj['maximum']}"</c:if><c:if test="${required == 1}"> required="required"</c:if>/>
</c:if>