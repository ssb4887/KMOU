				...
				<c:if test="${!empty itemSearchOrder}">
					...
					<c:forEach var="itemSearchId" items="${itemSearchOrder}">
						<c:choose>
							...
							<!-- 옵션 -->
							<c:set var="itemKeyFieldId" value="${keyFieldInfo.select_order[0]}"/>
							<c:set var="keyItem" value="${keyFieldInfo.items[itemKeyFieldId]}"/>
							<c:set var="itemKeyFieldName" value="${elfn:getItemName(keyItem)}"/>
							<input type="hidden" name="${keyFieldInfo.select_name}" id="${keyFieldInfo.select_name}" value="${itemKeyFieldId}">
							<input type="text" name="${keyFieldInfo.query_name}" id="${keyFieldInfo.query_name}" value="<c:out value="${queryString[keyFieldInfo.query_name]}"/>" class="form-control" title="검색어" placeholder="${itemKeyFieldName}를 입력해 주세요."/>
							</c:if>
							</c:when>
							<c:otherwise>
