...
<td<c:if test="${colspan > 0}"> colspan="${colspan}"</c:if>>
	<itui:objectPYMDH itemId="${itemId}" itemInfo="${itemInfo}" objDt="${objDt}" objVal11="${objVal11}" objVal12="${objVal12}" objVal21="${objVal21}" objVal22="${objVal22}" btnId="${btnId}" btnClass="${btnClass}" btnText="${btnText}" objClass="${objClass}" objStyle="${objStyle}"/>
</td>
</c:if>