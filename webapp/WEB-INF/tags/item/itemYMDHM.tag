...
<td<c:if test="${colspan > 0}"> colspan="${colspan}"</c:if>>
	<itui:objectYMDHM itemId="${itemId}" itemInfo="${itemInfo}" objDt="${objDt}" objVal1="${objVal1}" objVal2="${objVal2}" objVal3="${objVal3}" btnId="${btnId}" btnClass="${btnClass}" btnText="${btnText}" objClass="${objClass}" objStyle="${objStyle}"/>
</td>
</c:if>