...
<c:if test="${!empty itemObj}">
	...
	<%/* 관리시스템  || 관리시스템에서만 저장하는 항목 아닌 경우 */ %>
		...
		<th scope="row"><label for="${labelForId}"${labelId}<c:if test="${required == 1}"> class="required"</c:if>><c:if test="${!isAdmMode && required == 1}"><em class="point01">(필수)</em></c:if><itui:objectItemName itemId="${itemId}" itemInfo="${itemInfo}"/></label></th>
