...
<spring:message var="itemName" code="item.pre_password.name" arguments="${elfn:getItemName(itemObj)}"/>
<th scope="row"><itui:objectLabel itemId="${itemId}" required="1" itemInfo="${itemInfo}" id="pre_${itemId}" itemName="${itemName}"/></th>
<td<c:if test="${colspan > 0}"> colspan="${colspan}"</c:if>>
	<itui:objectPasswordPre itemId="${itemId}" itemInfo="${itemInfo}" objClass="${objClass}" objStyle="${objStyle}"/>
</td>
</c:if>