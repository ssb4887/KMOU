...
<c:forEach var="itemId" items="${itemOrder}">
	...
	<c:choose>
		...
		<c:if test="${itemObj.add_btn_type == 5}">
		try{$(".fn_btn_set_${itemKeyId}").click(function(event){fn_setCurrentDateObj2($(this).attr('data-idx'), $('#${itemKeyId}1'), $('#${itemKeyId}2'));return false;});}catch(e){}
		</c:if>
		</c:when>
	</c:choose>
