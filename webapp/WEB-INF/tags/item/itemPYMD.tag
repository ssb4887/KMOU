...
<td<c:if test="${colspan > 0}"> colspan="${colspan}"</c:if>>
	<itui:objectPYMD itemId="${itemId}" itemInfo="${itemInfo}" objDt="${objDt}" objVal1="${objVal1}" objVal2="${objVal2}" btnId="${btnId}" btnClass="${btnClass}" btnText="${btnText}" objClass="${objClass}" objStyle="${objStyle}"/>
</td>
</c:if>