...
<select id="${itemId}2" name="${itemId}2" style="${objStyle}" title="${quartObjTitle}"<c:if test="${required == 1}"> required="required"</c:if>>
	<option value="">${quartOptTitle}</option>
	<c:set var="optnOptList" value="${optnHashMap[itemObj['master_code']]}"/>
	<c:forEach var="optnDt" items="${optnOptList}" varStatus="i">
	<option value="<c:out value="${optnDt.OPTION_CODE}"/>"<c:if test="${!empty optnDt.P_OPT_CD}"> data-p="<c:out value="${optnDt.P_OPT_CD}"/>"</c:if><c:if test="${optnDt.OPTION_CODE == objVal2 || empty objVal2 && optnDt.OPTION_CODE == defVal2}"> selected="selected"</c:if><c:if test="${optnDt.OPTION_CODE == defVal2}"> data-default="1"</c:if>><c:out value="${optnDt.OPTION_NAME}"/></option>
	</c:forEach>
</select>
</c:if>${itemQuartName}