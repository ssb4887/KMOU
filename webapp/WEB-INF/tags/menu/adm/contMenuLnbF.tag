	...
	<c:if test="${!empty menuList}">
		...
		<c:forEach var="menuListInfo" items="${menuList}">
			...
			<li class="depth${level - 1} off">
				...
				<a style="display:none;" <c:if test="${empty menuLink}">class="no-link"</c:if> href="<c:out value="${menuLink}"/>" target="<c:out value="${target}"/>" title="<c:out value="${menuName}"/>"><c:out value="${menuName}"/></a>				
				</c:when>
				<c:otherwise>
