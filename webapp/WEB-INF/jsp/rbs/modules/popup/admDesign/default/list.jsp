	...
	<div id="cms_board_article">
		...
		<table class="tbListA" summary="<c:out value="${settingInfo.list_title}"/> 목록을 볼 수 있고 제목 링크를 통해서 게시물 상세 내용으로 이동합니다.">
			...
			<tbody class="alignC">
				...
				<c:set var="listKey" value="${listDt.POP_IDX}"/>
				<c:choose>
					<c:when test="${listDt.POP_TYPE == '2'}">
						<c:set var="linkUrl" value="${URL_VIEW}&${listIdxName}=${listKey}"/>
						<c:set var="beginALink">
							<a href="${linkUrl}" class="fn_pop_view" data-title="<c:out value="${listDt.SUBJECT}"/>" data-properties="<c:if test="${!empty listDt.POP_TOP}">top=<c:out value="${listDt.POP_TOP}"/>px<c:set var="isProperties" value="${true}"/></c:if><c:if test="${!empty listDt.POP_LEFT}"><c:if test="${isProperties}">,</c:if>left=<c:out value="${listDt.POP_LEFT}"/>px<c:set var="isProperties" value="${true}"/></c:if><c:if test="${isProperties}">,</c:if>width=<c:out value="${listDt.POP_WIDTH}"/>px,height=<c:out value="${listDt.POP_HEIGHT}"/>px">
						</c:set>
					</c:when>
					<c:when test="${listDt.POP_TYPE == '1' && !empty listDt.LINK_URL}">
						<c:set var="linkUrl" value="${listDt.LINK_URL}"/>
						<c:set var="beginALink">
							<a href="<c:out value="${linkUrl}"/>" target="${listDt.LINK_TARGET}" data-title="<c:out value="${listDt.SUBJECT}"/>">
						</c:set>
					</c:when>
				</c:choose>
				...
				<tr>
					...
					<td class="pop_img"><img src="<c:out value="${URL_IMAGE}"/>&id=${elfn:imgNSeedEncrypt(listDt.IMG_SAVED_NAME)}" alt="<c:out value="${listDt.IMG_TEXT}"/>"/></td>
					<td class="tlt">
						${beginALink}<c:out value="${listDt.SUBJECT}"/>${endALink}
					</td>
					<td><c:out value="${listDt.ISSTOP_NAME}"/></td>
					<td class="date"><c:if test="${!empty listDt.DSP_DATE11 || !empty listDt.DSP_DATE21}">
					<c:set var="objVal11" value="${listDt.DSP_DATE11}"/>
					<c:set var="objVal21" value="${listDt.DSP_DATE21}"/>
					<c:if test="${!empty objVal11}">
						<c:set var="objVal12" value=" ${listDt.DSP_DATE12}:${listDt.DSP_DATE13}"/>
					</c:if>
					<c:if test="${!empty objVal21}">
						<c:set var="objVal22" value=" ${listDt.DSP_DATE22}:${listDt.DSP_DATE23}"/>
					</c:if>
