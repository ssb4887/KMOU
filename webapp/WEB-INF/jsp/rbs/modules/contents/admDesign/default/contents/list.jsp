...
<c:set var="searchFormId" value="fn_contentsSearchForm"/>
<c:set var="listFormId" value="fn_contentsListForm"/>
...
<c:if test="${!empty TOP_PAGE}">
	...
	<div id="cms_board_article">
		...
		<div class="btnTopFull">
			<c:set var="input_dialog_height" value="500"/>
			<%@ include file="../../../../../adm/include/module/listBtns.jsp"%>
		</div>
		<!-- //button -->
		<%/* 콘텐츠 구분 사용여부 */ %>
		<c:set var="noListColspan" value="${7}"/>
		<%@ include file="../../common/useContTypeList.jsp"%>
		...
		<table class="tbListA" summary="<c:out value="${settingInfo.list_title}"/> 목록을 볼 수 있고 수정 링크를 통해서 수정페이지로 이동합니다.">
			...
			<colgroup>
				...
				<col width="70px" />
				<col width="70px" />
				<c:if test="${useContentsType}"><col width="150px" /></c:if>
				<col width="150px" />
				<col />
				<col width="100px" />
			</colgroup>
			...
			<tr>
				...
				<th scope="col">수정</th>
				<th scope="col">관리</th>
				<c:if test="${useContentsType}"><th scope="col">구분</th></c:if>
			...
			<tbody class="alignC">
				...
				<tr>
					<td colspan="${noListColspan}" class="bllist"><spring:message code="message.no.list"/></td>
				</tr>
				</c:if>
				...
				<tr>
					...
					<td><c:if test="${mngAuth}"><a href="<c:out value="${URL_BRANCHDEFAULT_LIST}"/>&${listIdxName}=${listKey}" attr-title="<c:out value="${listDt.CONTENTS_NAME}"/>" class="btnTypeE fn_btn_managelist">관리</a></c:if></td>
					<c:if test="${useContentsType}"><td><itui:objectView itemId="contType" itemInfo="${itemInfo}" objDt="${listDt}"/></td></c:if>
					<td><c:out value="${listDt.CONTENTS_NAME}"/></td>
