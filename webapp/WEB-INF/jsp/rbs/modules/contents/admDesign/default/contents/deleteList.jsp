...
<spring:message var="msBtnCalSel" code="button.calendar.select"/>
	<div id="cms_board_article">
		...
		<!-- //button -->
		<%/* 콘텐츠 구분 사용여부 */ %>
		<c:set var="noListColspan" value="${6}"/>
		...
		<table class="tbListA" summary="<c:out value="${settingInfo.list_title}"/> 삭제 목록을 볼 수 있습니다.">
			...
			<colgroup>
				<col width="50px" />
				<col width="120px" />
				<c:if test="${useContentsType}"><col width="100px" /></c:if>
				<col />
				<col width="90px" />
				<col width="90px" />
				<col width="90px" />
			...
			<tbody class="alignC">
				...
				<tr>
					<td colspan="${noListColspan}" class="bllist"><spring:message code="message.no.delete.list"/></td>
				</tr>
				</c:if>
