...
<c:set var="searchFormId" value="fn_itemClassMstrSearchForm"/>
<c:set var="listFormId" value="fn_itemClassMstrListForm"/>
		...
		<table class="tbListA" summary="<c:out value="${settingInfo.list_title}"/> 목록을 볼 수 있고 수정 링크를 통해서 수정페이지로 이동합니다.">
			...
			<tbody class="alignC">
				...
				<tr>
					...
					<td><c:if test="${mngAuth}"><a href="<c:out value="${URL_OPTNLIST}"/>&${listIdxName}=${listKey}" attr-title="<c:out value="${listDt.MASTER_NAME}"/>" class="btnTypeE fn_btn_optnlist">관리</a></c:if></td>
