			...
			<ul class="group">
				...
				<li>
					<a href="<c:out value="${URL_VIEW}"/>&${listIdxName}=${listKey}" title="상세보기" class="fn_btn_view">
						<span class="photo"><img src="<c:out value="${URL_IMAGE}"/>&id=<c:out value="${elfn:imgNSeedEncrypt(listDt.FILE_SAVED_NAME)}"/>" alt="<c:out value="${listDt.FILE_TEXT}"/>" onerror="$(this).remove();"/></span>
