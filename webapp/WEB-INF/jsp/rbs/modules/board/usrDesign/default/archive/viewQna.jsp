		...
		<%@ include file="../../../../../usr/web/include/commuTabMenu.jsp" %>	
			<div class="table-wrap2 mt50">
				...
				<c:if test="${listDt.RE_LEVEL <= 1 or (listDt.RE_LEVEL > 1 and listDt.REPLY_STATE eq 2)}">
					...
					<c:if test="${listDt.RE_LEVEL <= 1}">
						<div class="table-view">
							<div class="heading-line">
							    <p class="title">${listDt.SUBJECT}</p>
							    <div class="heading-etc">
							        <span class="txt">${listDt.NAME}</span>
							        <span class="txt">${listDt.REGI_DATE}</span>                                
							    </div>
							</div>
                	...
                	<c:if test="${listDt.RE_LEVEL > 1 and listDt.REPLY_STATE eq 2}">
		                <div class="table-view reply">
		                    <div class="heading-line">
		                        <p class="title">${listDt.SUBJECT}</p>
		                        <div class="heading-etc">
		                            <span class="txt">${listDt.NAME}</span>
			...
			<div class="btn-line mt30">
                <a href="<c:out value="${URL_QNA_LIST}"/>" title="목록" class="btnTypeA btn-large btn-primary" style="line-height:52px;">목록</a>
            </div>
        </div>
<%-- 		<c:set var="useFile" value="${settingInfo.use_file eq '1'}"/> --%>
<%-- 		<c:set var="useReply" value="${settingInfo.use_reply eq '1'}"/> --%>
