	...
	<jsp:include page="${TOP_PAGE}" flush="false">
		<jsp:param name="javascript_page" value="${moduleJspRPath}/view.jsp"/>
			...
			<table class="table noti_view">
				<caption class="blind">취업정보 상세정보</caption>
				<colgroup>
					<col width="15%" />
					<col width="35%" />
					<col width="15%" />
				...
				<tbody>
					<tr>
						...
						<td class="date text-start align-middle"><fmt:formatDate pattern="YYYY.MM.dd" value="${dt.REG_DT}"/></td>
					</tr>
			...
			<div class="btn-line mt30">
				<a href="<c:out value="${URL_LIST}"/>" title="목록"class="back_to_list">목록</a>
            	...
            	<c:forEach var="listDt" items="${list }" varStatus="i">
            		<c:choose>
            			<c:when test="${len eq '2'}">
            				<c:choose>
        						<c:when test="${i.first }">
									<li><a href="/web/board/view.do?mId=43&page=1&NTT_SN=${listDt.NTT_SN }"><strong>이전글</strong><span>${listDt.NTT_SJ }</span></a></li>
								</c:when>
        						<c:when test="${i.last }">
        							<li><a href="/web/board/view.do?mId=43&page=1&NTT_SN=${listDt.NTT_SN }"><strong>다음글</strong><span>${listDt.NTT_SJ }</span></a></li>
								</c:when>
