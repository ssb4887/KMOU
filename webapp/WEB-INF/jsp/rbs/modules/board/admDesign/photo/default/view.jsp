	...
	<div id="cms_board_article">
		...
		<table class="tbViewA" summary="${summary}">
			...
			<tbody>
				...
				<tr>
					<th scope="row"><spring:message code="item.file.name"/></th><td colspan="5"><itui:objectView itemId="file" multiFileHashMap="${multiFileHashMap}" imgDisplayType="${1}"/></td>
				</tr>
				</c:if>
				<itui:itemViewAll colspan="5" itemInfo="${itemInfo}" itemOrderName="${submitType}_order" exceptIds="${exceptIds}"/>
				<c:if test="${settingInfo.use_file == '1'}">
				<tr>
					<td class="cont fn_img" colspan="6"><itui:objectView itemId="file" multiFileHashMap="${multiFileHashMap}"/></td>
				</tr>
				</c:if>
				<tr>
