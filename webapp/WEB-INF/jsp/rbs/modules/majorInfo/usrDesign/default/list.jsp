...
<c:if test="${!empty TOP_PAGE}">
	...
	<h1>11111111111111</h1>
		<h2 class="title">기술지원신청현황1111111111</h2>
		<!-- search -->
		<itui:searchFormItem divClass="tbMSearch fn_techSupportSearch" formId="${searchFormId}" formName="${searchFormId}"
			formAction="${URL_DEFAULT_LIST}" itemListSearch="${itemInfo.list_search}" searchOptnHashMap="${searchOptnHashMap}" searchFormExceptParams="${searchFormExceptParams}"
			submitBtnId="fn_btn_search" submitBtnClass="btnSearch" submitBtnValue="조회" listAction="${URL_DEFAULT_LIST}" listBtnId="fn_btn_totallist" listBtnClass="btnTotalList"/>
		<!-- //search -->
		<div class="bbs_top">
		...
		<table class="listTypeA" summary="<c:out value="${settingInfo.list_title}"/> 목록을 볼 수 있고 수정 링크를 통해서 수정페이지로 이동합니다.">
			...
			<tbody class="alignC">
				...
				<tr>
					...
					<td><itui:objectView itemId="subject" itemInfo="${itemInfo}" objDt="${listDt}"/></td>
					...
					<td><fmt:parseDate value="${listDt.REGI_DATE}" pattern="yyyy-MM-dd'T'HH:mm" var="parsedDate" type="both"/><fmt:formatDate pattern="yyyy-MM-dd" value="${parsedDate}"/></td>
				</tr>
