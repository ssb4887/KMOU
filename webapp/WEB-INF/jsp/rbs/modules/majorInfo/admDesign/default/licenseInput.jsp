...
<c:set var="itemObjs" value="${itemInfo.items}"/>
	<div id="cms_board_article">
		...
		<form id="${inputFormId}" name="${inputFormId}" method="post" action="<c:out value="${URL_LICENSE_INPUT_PROC}"/>" target="submit_target" enctype="multipart/form-data">
			...
			<table id="tpSubContent" class="tbListA" >
				...
				<thead>
			        <tr>
			            ...
			            <th scope="row"><strong>*</strong> 관련자격증</th>
			        </tr>		
				</thead>
				<tbody class="alignC">
					<c:choose>
						<c:when test="${empty list}">
							<c:forEach var="listDt" items="${code}" varStatus="i">
			                	<tr>
			                		...
			                		<td>
			                			<textarea placeholder="관련자격증" class="inputTxt" name="licenseNm${i.index}" id="licenseNm${(i.index)+1}"></textarea>
									</td>
						...
						<c:otherwise>
			               	<c:forEach var="listDt" items="${list}" varStatus="i">
			               		...
			               		<tr>
			                		<td><input type="hidden" value="${i.count}" name="ord${i.index}" id="ord${i.count}" class="w100"><span id="ord">${i.count}</span></td>
			                		<td>
			                			<input type="hidden" value="${listDt.FIELD }" name="fieldCd${i.index}" id="fieldCd${i.count}" class="w100">
			                			<input type="hidden" value="${listDt.FIELD }" name="field${i.index}" id="field${i.count}" class="w100">
			                			<span id="field">${listDt.FIELD }</span>
			                		</td>
			                		<td>
			                			<textarea placeholder="관련자격증" class="inputTxt" name="licenseNm${i.index}" id="licenseNm${(i.index)+1}" >${listDt.LICENSE_NM}</textarea>
									</td>
									<c:if test="${i.last == true }"><input type="hidden" value="${i.count }" name="fieldRowLength"></c:if>
			                	</tr>
			               	</c:forEach>
