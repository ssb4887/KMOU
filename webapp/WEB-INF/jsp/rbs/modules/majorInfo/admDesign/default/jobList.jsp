...
<%-- 		<a href="<c:out value="${URL_JOB_INPUT}"/>" title="추가" class="btnTW fn_btn_write${inputWinFlag}">추가</a> --%>
		<!-- //button -->
...
<%-- 				<th scope="col"><spring:message code="item.regidate.name"/></th> --%>
			...
			<tbody class="alignC">
				...
				<c:set var="listJobIdxName" value="${settingInfo.idx_job_name}"/>
...
<%-- 					<td><c:if test="${mngAuth || wrtAuth && listDt.AUTH_MNG == '1'}"><input type="checkbox" id="select" name="select" title="<spring:message code="item.select"/>" value="${listKey}"/></c:if></td> --%>
					...
					<td><a href="<c:out value="${URL_JOB_INPUT}"/>&mode=m&${listMajorIdxName}=${listDt.MAJOR_IDX}&${listYearIdxName}=${listDt.YEAR_IDX}&${listJobIdxName}=${listDt.JOB_IDX}" >${listDt.TITLE}</a></td>
<%-- 					<a href="javascript:void(0)" onclick='location.href="<c:out value="${URL_MODIFY}"/>&${listMajorIdxName}=${listDt.MJ_CD}&${listYearIdxName}=${listDt.YY}"' class="btnTypeF fn_btn_modify">수정</a> --%>
					<td>${listDt.USE_YN }</td>
