...
<jsp:include page="${jspSiteIncPath}/dialog_top.jsp" flush="false">
	<jsp:param name="page_tit" value="${itemSettingInfo.deleteList_title} (${queryString.tit})"/>
	<jsp:param name="javascript_page" value="${moduleJspRPath}/item/deleteList.jsp"/>
...
<spring:message var="msBtnCalSel" code="button.calendar.select"/>
	<div id="cms_board_article">
		<!-- search -->
		<itui:searchDelFormItem divClass="tbSearch tbShowDt" formId="${searchFormId}" formName="${searchFormId}" formAction="${URL_DEFAULT_LIST}" itemListSearch="${itemInfo.list_search}" searchOptnHashMap="${searchOptnHashMap}" searchFormExceptParams="${searchFormExceptParams}" submitBtnId="fn_btn_search" submitBtnClass="btnSearch" submitBtnValue="검색" listAction="${URL_DEFAULT_LIST}" listBtnId="fn_btn_totallist" listBtnClass="btnTotalList"/>
		<!-- //search -->
		...
		<table class="tbListA" summary="<c:out value="${settingInfo.list_title}"/> 삭제 목록을 볼 수 있습니다.">
			...
			<tbody class="alignC">
				...
				<tr>
					<td colspan="9" class="bllist"><spring:message code="message.no.delete.list"/></td>
				</tr>
				</c:if>
