...
<c:if test="${!empty TOP_PAGE}">
	<jsp:include page="${TOP_PAGE}" flush="false">
		<jsp:param name="javascript_page" value="${moduleJspRPath}/snsLogList.jsp"/>	
...
<div id="cms_board_article">
	...
	<c:set var="summary">번호, 사이트명, 메뉴명, 공유유형, 아이디, 성명, 일자, IP의  <c:out value="${crtMenu.menu_name}"/> 목록입니다.</c:set>
	<table class="tbListA tbContact" summary="${summary}">
		...
		<c:if test="${empty list}">
			<tr>
				<td colspan="7" class="nolist"><spring:message code="message.no.log.list"/></td>
			</tr>
		</c:if>
		...
		<c:forEach var="listDt" items="${list}" varStatus="i">
			<tr>
				...
				<td><c:out value="${listDt.SNS_TYPE}"/></td>
