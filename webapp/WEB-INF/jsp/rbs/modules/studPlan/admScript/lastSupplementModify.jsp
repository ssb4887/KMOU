...
// 	        html += '<td class="border-end tdPnt">' + totalPnt + '</td>';
	...
	function save(){
		if(confirm("저장하시겠습니까?")){
			...
			$.ajax({
				url: '/RBISADM/menuContents/web/studPlan/lastSupplementSave.do?mId=48',
				beforeSend:function(request){request.setRequestHeader('Ajax', 'true');},
				contentType:'application/json',
				type:'POST', 
	...
	/*수정 마감*/	
	function complete(){
		if(confirm("수정마감하시겠습니까? 수정마감 시 해당 학생설계전공은 최종승인되며, 종합정보시스템으로 전송됩니다.")){
	        let json_data = {};
	        getInputDataHasName(json_data);
	        
			$.ajax({
				url: '/RBISADM/menuContents/web/studPlan/lastSupplementComplete.do?mId=48',
				beforeSend:function(request){request.setRequestHeader('Ajax', 'true');},
				contentType:'application/json',
				type:'POST', 
				data: JSON.stringify({
				...
				success:function(data){				
					...
					if(data.RESULT == "DONE"){
						...
						$("#SDM_CD").val(json_data.SDM_CD);
						$("#REVSN_NO").val(json_data.REVSN_NO);
						form.action = '/RBISADM/menuContents/web/studPlan/lastSupplementList.do';
						form.submit();
					}
				}
			});	
