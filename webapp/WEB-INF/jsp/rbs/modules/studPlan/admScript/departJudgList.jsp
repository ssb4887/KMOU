...
<script type="text/javascript">
	...
	function getDepartJudgList(pageNo){
		...
		var selectedBox = ($("#selectMj").val() != '' ? $("#selectMj").val() : ($("#selectDept").val() != '' ? $("#selectDept").val() : ($("#selectColg").val() != '' ? $("#selectColg").val() : '')));
       	...
       	$.ajax({
			url : '/RBISADM/menuContents/web/studPlan/getDepartJudgList.do?mId=48',
			beforeSend:function(request){request.setRequestHeader('Ajax', 'true');},
			contentType:'application/json',
			type:'POST', 
			data: JSON.stringify({ 
	...
	function renderStudPlanList(data){
        ...
        if (list.length === 0) {
            ...
            $.each(list, function(index, listDt) {
                ...
                html += '<td class="date">' + listDt.EDU_CENTER_JUDG_DT + '</td>';                
		...
		if(totalPageCount > pageSize){			
			...
			if(currentPageNo != firstPageNo){
				pageCon += '<li class="page-item page-first"><a href="javascript:getDepartJudgList(' + firstPageNo + ');" class="page-link" title="first"><span class="blind">first</span><img src="<c:out value="${contextPath}${imgPath}/arr_2x_gray.png"/>" alt="처음으로 화살표" /></a></li>';
				pageCon += '<li class="page-item page-prev"><a href="javascript:getDepartJudgList(' + prevPageNo + ');" class="page-link" title="prev"><span class="blind">prev</span><img src="<c:out value="${contextPath}/${crtSiteId}/images/arr_left_gray.png"/>" alt="이전으로 화살표" /></a></li>';
			}
		...
		for(var pageNo = firstPageNoOnPageList; pageNo <= lastPageNoOnPageList; pageNo++){
			if(pageNo == currentPageNo){
				pageCon += '<li class="page-item active"><a class="page-link" href="javascript:getDepartJudgList(' + pageNo + ');" title="' + pageNo + '페이지" value="' + pageNo + '">' + pageNo + '</li>';
			} else{
				pageCon += '<li class="page-item"><a class="page-link" href="javascript:getDepartJudgList(' + pageNo + ');" title="' + pageNo + '페이지" value="' + pageNo + '">' + pageNo + '</a></li>';
			}
		}
  		...
  		if(totalPageCount > pageSize){
			...
			if(currentPageNo != lastPageNo){
				pageCon += '<li class="page-item page-next"><a href="javascript:getDepartJudgList(' + nextPageNo + ');" class="page-link" title="next"><span class="blind">next</span><img src="<c:out value="${contextPath}/${crtSiteId}/images/arr_left_gray.png"/>" alt="다음으로 화살표" /></a></li>';
	  			pageCon += '<li class="page-item page-last"><a href="javascript:getDepartJudgList(' + lastPageNo + ');" class="page-link" title="last"><span class="blind">last</span><img src="<c:out value="${contextPath}${imgPath}/arr_2x_gray.png"/>" alt="이전으로 화살표" /></a></li>';
			}
	...
	function goModify(a,b,c){
		...
		$("#SDM_CD").val(a);
		$("#REVSN_NO").val(b);
		form.action = '/RBISADM/menuContents/web/studPlan/depart'+isChg+'JudgModify.do';
		form.submit();
	}	
