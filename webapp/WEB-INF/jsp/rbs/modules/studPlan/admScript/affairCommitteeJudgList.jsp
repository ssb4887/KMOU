...
<script type="text/javascript">
	...
	function getAffairCommitteeList(pageNo){
		...
		var selectedBox = ($("#selectMj").val() != '' ? $("#selectMj").val() : ($("#selectDept").val() != '' ? $("#selectDept").val() : ($("#selectColg").val() != '' ? $("#selectColg").val() : '')));
       	...
       	$.ajax({
			url : '/RBISADM/menuContents/web/studPlan/getAffairCommitteeJudgList.do?mId=48',
			beforeSend:function(request){request.setRequestHeader('Ajax', 'true');},
			contentType:'application/json',
			type:'POST', 
			data: JSON.stringify({ 
	...
	function goModify(a,b,c){
		...
		$("#SDM_CD").val(a);
		$("#REVSN_NO").val(b);
		form.action = '/RBISADM/menuContents/web/studPlan/affairCommittee'+isChg+'JudgModify.do';
		form.submit();
	}	
