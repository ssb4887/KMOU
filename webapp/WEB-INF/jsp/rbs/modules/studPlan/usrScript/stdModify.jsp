...
let duplSbjtArr = [];
	...
	$(function(){
		...
		//교과목 설계범위 체크박스 세팅
		switch("${INFO.SBJT_DGN_RNG_FG}"){
			case "IMC":  $("#internalMajorCourse").prop("checked", true); break;
			case "IMG":  $("#internalMajorAndGeneral").prop("checked", true); break;
			case "IME":  $("#internalMajorAndExternal").prop("checked", true); break;
			case "IMT":  $("#internalMajorAndTeach").prop("checked", true); break;
		}
...
// 			var shyrFg = $(this).attr("aria-COM_GRADE");
		...
		$(".otherColg").each(function(){
			...
			obj.DEPT_CD 			= '';	 							 				// 개설학부코드         
			obj.DEPT_NM 			= $("#headAfter-"+num).attr("aria-COLG_NM");	 	// 개설학부한글명	 	
...
// ==========================================================================================================교수 검색=================================================================	
	...
	function profAddBtn(e){
		...
		if(e == 1){
			...
			$("#prof_1").css("display", "block");
			$("#prof_2").css("display", "block");
...
// 				중복 체크
	...
	function renderSbjtTable(g, grade){
		if(g != null){
		    ...
		    teachHtml =  processSbjtHtml(g, 'UE010031', '교직', teachRowSpan);
...
// 		}
	...
	function modifyTempSave(){
		...
		$.ajax({
			url: '/web/studPlan/modifyTempSave.do?mId=36',
			beforeSend:function(request){request.setRequestHeader('Ajax', 'true');},
			contentType:'application/json',
			type:'POST', 
...
// 					form.action = '/web/studPlan/stdModify.do';
	...
	/*학생설계전공 상담신청(수정)*/
	function modifyReqCnslt(){
		if(!basicValidCheck()) return false;
// 		if (!detailValidCheck()) return false;
        ...
        if(confirm("해당 학생설계전공에 대한 상담을 신청하시겠습니까?")){
			...
			$.ajax({
				url: '/web/studPlan/modifyReqCnslt.do?mId=36',
				beforeSend:function(request){request.setRequestHeader('Ajax', 'true');},
				contentType:'application/json',
				type:'POST', 
				data: JSON.stringify(json_data),
...
// 		if (!detailValidCheck()) return false;	//이동근 임시주석
		...
		if(confirm("해당 학생설계전공 신청서를 제출하시겠습니까?")){
			...
			$.ajax({
				url: '/web/studPlan/modifySubmit.do?mId=36',
				beforeSend:function(request){request.setRequestHeader('Ajax', 'true');},
				contentType:'application/json',
				type:'POST', 
				data: JSON.stringify(json_data),
...
// 						form.action = '/web/studPlan/stdMyView.do';
	...
	function setSavedSbjtToModal(data){
		...
		for(var i=0; i < data.length; i++){
			...
			html +=    '<section class="d-inline-flex flex-column flex-lg-row justify-content-between">';
			html +=        '<div class="d-flex flex-column align-items-start div-after">';
			html +=            '<ul class="cate d-flex flex-wrap gap-2 stdpl_modal_ul01">';
			html +=                sbjtFgHtml;
			html +=                '<li class="border stdpl_modal_label01">';
		...
		//교육과정표 및 설계학점 총계 세팅(모달의 저장버튼 클릭)
		if(data.length != 0){
			sbjtDesignScope();	
		}
	}
