...
let sbjtArray = new Array();
	$(function(){
	    ...
	    $('#rjtResn').on('input', function() {
	        $('input[name="RJT_RESN"]').val($(this).val());
	    });
...
// 	        html += '<td class="border-end tdPnt">' + totalPnt + '</td>';
	...
	function approve(){
		if(confirm("승인하시겠습니까?")){
			...
			$.ajax({
				url: '/web/studPlan/profApprove.do?mId=36',
				beforeSend:function(request){request.setRequestHeader('Ajax', 'true');},
				contentType:'application/json',
				type:'POST', 
				data: JSON.stringify({
...
// 						form.action = '/web/studPlan/profApproveChgModify.do';
	...
	/*학생설계전공 제출 반려*/	
	function reject(){
		if($('input[name="RJT_RESN"]').val() === undefined || $('input[name="RJT_RESN"]').val() === ''){
			alert("반려 시 반려사유는 필수 입력입니다.");
			return false;
		}
		if(confirm("반려하시겠습니까?")){
	        let json_data = {};
			...
			$.ajax({
				url: '/web/studPlan/profReject.do?mId=36',
				beforeSend:function(request){request.setRequestHeader('Ajax', 'true');},
				contentType:'application/json',
				type:'POST', 
				data: JSON.stringify({
