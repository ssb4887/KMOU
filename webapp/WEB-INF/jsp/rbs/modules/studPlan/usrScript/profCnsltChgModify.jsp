...
let sbjtArray = new Array();
	$(function(){
		...
		// 각 항목의 입력마다 hidden에 동기화
	    $('#configCurric').on('input', function() {
	        $('input[name="SBJT_CNST_ADV_OPIN"]').val($(this).val());
	    });
	    
	    $('#majorNName').on('input', function() {
	        $('input[name="MJ_NM_ADV_OPIN"]').val($(this).val());
	    });
	    
	    $('#awwwardDegrre').on('input', function() {
	        $('input[name="AWD_DEGR_NM_ADV_OPIN"]').val($(this).val());
	    });
	    
	    $('#reqMeetMsg').on('input', function() {
	        $('input[name="MEET_CNSLT_ADV_OPIN"]').val($(this).val());
	    });
...
// 	        html += '<td class="border-end tdPnt">' + totalPnt + '</td>';
	...
	/*임시저장*/	
	function cnsltTempSave(){
        let json_data = {};
        getInputDataHasName(json_data);
        json_data['ACTION'] = 'TEMPSAVE'; 
		console.log(json_data);
		$.ajax({
			url: '/web/studPlan/profCnsltAction.do?mId=36',
			beforeSend:function(request){request.setRequestHeader('Ajax', 'true');},
			contentType:'application/json',
			type:'POST', 
			...
			success:function(data){							
				...
				if(data.RESULT == "DONE"){
					...
					form.action = '/web/studPlan/profCnsltModify.do';
					form.submit();
				}else{
	...
	/*상담완료*/	
	function cnsltComplete(){
        let json_data = {};
        getInputDataHasName(json_data);
        json_data['ACTION'] = 'COMPLETE';
        var a = $('input[name="SBJT_CNST_ADV_OPIN"]').val()
        var b = $('input[name="MJ_NM_ADV_OPIN"]').val()
        var c = $('input[name="AWD_DEGR_NM_ADV_OPIN"]').val()
		if ((a == undefined || a == '') && (b == undefined || b == '') && (c == undefined || c == '')) {
			alert("최소 1개 항목 이상의  지도의견을 입력해 주세요.");				
			return false;
		}
		
		if(confirm("요청된 상담을 완료처리 하시겠습니까?")){
			...
			$.ajax({
				...
				success:function(data){							
					...
					if(data.RESULT == "DONE"){
						...
						$("#SDM_CD").val(json_data.SDM_CD);
						$("#REVSN_NO").val(json_data.REVSN_NO);
// 						form.action = '/web/studPlan/profCnsltView.do';
	...
	/*방문상담요청*/	
	function cnsltRequestMeeting(){
        let json_data = {};
        getInputDataHasName(json_data);
        json_data['ACTION'] = 'MEET';
        
		var a = $('input[name="MEET_CNSLT_ADV_OPIN"]').val(); 
        if(a == undefined || a == ''){
        	alert("방문상담요청 시 방문상담입력란은 필수입력입니다.");
        	return false;
        }
		
		if(confirm("방문상담요청 시 상담은 완료처리됩니다. 방문상담요청 하시겠습니까?")){
