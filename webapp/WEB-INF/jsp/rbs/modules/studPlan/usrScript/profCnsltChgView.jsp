...
let sbjtArray = new Array();
	$(function(){
	    ...
	    $('#reqMeetMsg').on('input', function() {
	        $('input[name="MEET_CNSLT_REQ_OPIN"]').val($(this).val());
	    });
...
// 	        html += '<td class="border-end tdPnt">' + totalPnt + '</td>';
	...
	function cnsltRequestMeeting(){
		...
		var meetCnsltReqOpin = $('input[name="MEET_CNSLT_REQ_OPIN"]').val(); 
        if(meetCnsltReqOpin == undefined || meetCnsltReqOpin == ''){
        	alert("방문상담요청 시 방문상담입력란은 필수입력입니다.");
        	return false;
        }
		
		if(confirm("방문상담요청 하시겠습니까?")){
