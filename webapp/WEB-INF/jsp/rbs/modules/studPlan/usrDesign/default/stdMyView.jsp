...
<c:if test="${!empty TOP_PAGE}">
	<jsp:include page="${TOP_PAGE}" flush="false">
		<jsp:param name="javascript_page" value="${moduleJspRPath}/usrScript/stdMyView.jsp"/>
	</jsp:include>
</c:if>
			...
			<section class="plan_dtl_section plan_dtl_basic" id="move_acitve2">
				...
				<div class="d-flex flex-wrap table_wrap">
					...
					<div class="d-grid col-12 col-md-6">
						...
						<h4 class=" d-flex justify-content-start align-items-center substanc fw-normal px-3 py-1 border-bottom border-end">국문명</h4>
						<p class=" d-flex justify-content-start align-items-center border-bottom border-end px-3 py-2">
							${INFO.AWD_DEGR_KOR_NM }
						</p>
						<h4 class=" d-flex justify-content-start align-items-center substanc fw-normal px-3 py-1 border-bottom border-end">영문명</h4>
						<p class=" d-flex justify-content-start align-items-center border-bottom border-end px-3 py-2">
							${INFO.AWD_DEGR_ENG_NM }
						</p>
					</div>				
				</div>
			</section>								
					...
					<div class="search_proff d-flex flex-column flex-md-row mt-3">
						<dl class="d-flex flex-row col-12 col-md-6 align-items-center">
							...
							<dd class="col-8 col-sm-9 d-flex flex-row px-3 py-2 gap-2">
								<input id="proNm" type="text" name="GUID_PROF_STAFF_NM" class="form-control" placeholder="-" value="${INFO.GUID_PROF_STAFF_NM}" readonly>
						...
						<dl class="d-flex flex-row col-12 col-md-6 align-items-center">
							...
							<dd class="col-8 col-sm-9 d-flex flex-row px-3 py-2 gap-2">
								<input id="proConNm" type="text" name="CNSLT_PROF_NM" class="form-control" placeholder="-" value="${INFO.CNSLT_PROF_NM}" readonly>
...
<!-- 			컨설팅 결과 -->
			...
			<section class="plan_proces ">
				<ul class="d-flex flex-wrap justify-content-center" id="progress">
					<li class="px-3 py-2 rounded-pill reject" id="33" style="display:none;">해당 학생설계전공은 지도교수님에 의해 반려되었습니다.</li>	
					<li class="px-3 py-2 rounded-pill reject" id="43" style="display:none;">해당 학생설계전공은 창의융합교육센터에 의해 반려되었습니다.</li>	
					<li class="px-3 py-2 rounded-pill reject" id="53" style="display:none;">해당 학생설계전공은 학사과에 의해 반려되었습니다.</li>	
					<li class="px-3 py-2 rounded-pill reject" id="63" style="display:none;">해당 학생설계전공은 교무회 및 평의원회에 의해 반려되었습니다.</li>	
					<li class="px-3 py-2 rounded-pill reject" id="20">상담 진행중</li>
					<li class="px-3 py-2 rounded-pill" id="25">상담 완료(마감)</li>
					<li class="px-3 py-2 rounded-pill" id="30">지도교수 승인 대기중</li>
					<li class="px-3 py-2 rounded-pill" id="40">최종 승인 대기중</li>
					<li class="px-3 py-2 rounded-pill" id="100">최종 승인 완료</li>	
				</ul>
			</section>				
			<section class="plan_button_wrap d-flex flex-wrap justify-content-center gap-2 gap-md-3 mt-4">
				<c:set var="modifiableList" value="25,33,43,53,63" />
				<c:set var="cnsltClosableList" value="20" />
				<c:set var="changableList" value="100" />
				<!-- 컨설팅 마감, 지도교수 반려 : 수정 가능 -->
				<c:if test="${fn:contains(modifiableList, INFO.STATUS)}">
				<button onclick="updateStatus('10','수정가능상태로 변경')" type="button" class="border-0 py-3" style="background-color: #309CBE;">수정</button>
				</c:if>
				<!-- 상담 신청 : 상담 마감 가능 -->
				<c:if test="${INFO.STATUS eq '20'}">
				<button onclick="updateStatus('10','상담 마감')" type="button" class="border-0 py-3" style="background-color: #309CBE;">상담 마감</button>
				</c:if>
				<!-- 최종승인+전공코드존재+변경가능Y : 변경 등록 가능 -->
				<c:if test="${INFO.STATUS eq '100' and !empty INFO.SDM_DEPT_CD and INFO.IS_CHG_YN eq 'Y'}">
				<button onclick="changeInput('${INFO.SDM_CD}','${INFO.REVSN_NO }')" type="button" class="border-0 py-3" style="background-color: #309CBE;">변경 등록</button>
				</c:if>
			</section>
	</section>
</div>
