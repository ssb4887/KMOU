...
<c:if test="${!empty TOP_PAGE}">
	<jsp:include page="${TOP_PAGE}" flush="false">
		<jsp:param name="javascript_page" value="${moduleJspRPath}/usrScript/profApproveView.jsp"/>
	</jsp:include>
</c:if>
...
<!-- <div class="container_wrap"> -->
	<div class="sub_wrap">
		<div class="sub_background plan_bg">
			<section class="inner">
				<h3 class="title fw-bolder text-center">학생설계전공${TOP_PAGE}</h3>
			</section>
		</div>
		<!--본문-->
			...
			<section class="detail_title_wrap">
				<section class="d-flex flex-row justify-content-between align-items-center title_box px-2 py-3 mb-5">
					...
					<h5 class="content_title text-center fw-bolder d-flex align-items-center justify-content-center gap-1 flex-wrap">
						<span class="text-truncate">${INFO.SDM_KOR_NM }<c:if test="${INFO.STATUS eq '33'}">(반려)</c:if><c:if test="${INFO.STATUS gt '33' }">(승인)</c:if></span>
					</h5>
				</section>
			</section>
...
<!-- 										<th scope="col" class="border-end">전체</th> -->
	...
	<!-- 			반려 사유 -->
				<c:if test="${INFO.STATUS eq '33' }">
				<div id="move_acitve4" class="plan_dtl_section">
					<section class=" plan_result  plan_result_proff">
						<h5 class="fw-bold mb-3">반려 사유</h5>
						<div class="table_wrap">
							<table class="table">
								<caption class="blind">반려 사유</caption>
								<colgroup>
									<col width="15%"/>
									<col width="auto"/>
								</colgroup>
								<tbody>
									<tr>
										<td class="open_maj text-center align-middle">반려 사유</td>
										<td class="guid_opi text-start align-middle">
											<textarea class="form-control border-bottom" rows="4" placeholder="-" id="rjtResn" disabled>${INFO.RJT_RESN }</textarea>
											<label for="rjtResn" class="blind">반려 사유</label>      
											<input type="hidden" name="RJT_RESN" value="">
										</td>
									</tr>
								</tbody>
							</table>
