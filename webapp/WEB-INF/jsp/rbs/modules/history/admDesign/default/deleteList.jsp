...
<c:set var="searchFormId" value="fn_historySearchForm"/>								<%/* 검색폼 id/name */ %>
<c:set var="listFormId" value="fn_historyListForm"/>									<%/* 목록관리폼 id/name */ %>
		...
		<table class="tbListA" summary="<c:out value="${settingInfo.list_title}"/> 삭제 목록을 볼 수 있습니다.">
			...
			<tbody class="alignC">
				...
				<tr>
					...
					<td><c:out value="${listDt.HIS_YEAR}"/>년<c:out value="${listDt.HIS_MONTH}"/>월</td>
					<td><itui:objectView itemId="contents" itemInfo="${itemInfo}" objDt="${listDt}"/></td>
