...
<head>
	...
	<title><c:choose><c:when test="${!empty siteInfo}">${siteInfo.site_title}</c:when><c:otherwise><spring:message code="Globals.site.default.title"/></c:otherwise></c:choose> - 잘 못된  페이지 접근입니다.</title>
	<script type="text/javascript">
	try{
		if(window != top && window.innerWidth == 0) {
			alert("잘 못된  페이지 접근입니다.");
		}
	}catch(e){}
	</script>
</head>
<body>
	<h1>잘 못된  페이지 접근입니다.</h1>
