function createSEditor2(elIRField, htParams, elSeAppContainer){
	...
	var varSize;
	var varoIRHeight = oIRTextarea.style.height;
	var varHeight;
	if(varoIRHeight) 
	{
		try {
			varHeight = new Number(varoIRHeight.replace('px', '')) * 1.5;
		} catch(e){}
	}
	else varHeight = 355;
	try
	{
		if(parent && parent.gVarEditorSize) varSize = parent.gVarEditorSize;
		else varSize = [620, varHeight];
	}
	catch (e)
	{
		varSize = [620, varHeight];
	}
	var htDimension = {
		nMinHeight:varSize[1],
		nMinWidth:parseInt(elIRField.style.minWidth, 10)||varSize[0], //570,
