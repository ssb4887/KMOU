	 ...
	 */
     function goStartMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") < 0 ){
    		 welBtnConfirm.attr("src",sSrc.replace("btn_confirm.png", "btn_confirm2.png"));
     ...
     function goReadyMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") >= 0 ){
    		 ...
    		 welBtnConfirm.attr("src",sSrc.replace("btn_confirm2.png", "btn_confirm.png"));
    ...
    function html5Upload() {	
    	...
    	sUploadURL= 'file_uploader_html5.jsp' + gVarQuery; 	//upload URL
    	...
    	for(var j=0, k=0; j < nImageInfoCnt; j++) {
    		...
    		try{
	    		if(!!tempFile){
	    			...
	    			callAjaxForHTML5(tempFile,sUploadURL, k);
	    			k += 1;
	    		}
	    	}catch(e){}
    		tempFile = null;
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL, theIdx){
    	//alert(tempFile + ':' + sUploadURL);
 		...
 		try{
	    	...
	    	for (var i = 0; i < aTemp.length ; i++){
	    		if( !!aTemp[i] && aTemp[i] != "" && aTemp[i].indexOf("=") > 0){
					...
					/*if(varFIdx == 0) {
						aResultleng = new Number(aSubTemp[1]);
						varFIdx ++;
					} else {*/
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			//sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/file_uploader.jsp' + gVarQuery,	//샘플 URL입니다. 우원 cms customizing : 2015.04.27
 			sUrl	: gVarSPath + '/editor/photoUploadProc.do' + gVarQuery,
 	        sCallback : gVarCPath + '/callback.html', //location.href.replace(/\/[^\/]*$/, '') + '/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
 	    	...
 	    	select : function(oCustomEvent) {
 	    		//alert(gVarSPath + '/editor/photoUploadProc.do' + gVarQuery.replace(/&amp;/g, '&'));
 	    		//파일 선택이 완료되었을 때 발생
// 		    	 oCustomEvent (이벤트 객체) = {
